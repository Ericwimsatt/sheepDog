...
describe('statusCommand', () => {
    let tmpDir;
    function writeTask(name, phaseCount = 4) {
    ...
    function writeTodoFiles(taskDir, count) {
        for (let i = 1; i <= count; i++) {
            writeFileSync(join(taskDir, `todo-phase-${i}.md`), `# Phase ${i}`, 'utf-8');
        }
    }
    function setActivePhase(taskDir, phaseId) {
        writeFileSync(join(taskDir, '.active-phase'), phaseId, 'utf-8');
    }
    function createContextFile(taskDir, phaseId) {
