...
const phase = { id: 'p1', file: 'todo-p1.md', label: 'Phase 1', runAfter: [] };
function writeTodoFile() {
    writeFileSync(join(tmpDir, 'todo-p1.md'), 'Do the work\n', 'utf-8');
}
function scheduleDoneMarker() {
...
describe('PhaseRunner', () => {
    ...
    it('calls herdr.startAgent with correct arguments', async () => {
        ...
        expect(mockHerdr.startAgent).toHaveBeenCalledWith('Sheepdog (p1)', '/project', ['opencode', '--prompt', expect.stringContaining('Phase "Phase 1" has started'), '--auto'], { split: 'right' });
    });
...
describe('PhaseRunner.runFixAttempt', () => {
    ...
    it('launches fix agent with correct arguments', async () => {
        ...
        expect(mockHerdr.startAgent).toHaveBeenCalledWith('Sheepdog fix (p1, attempt 1)', '/project', expect.arrayContaining(['opencode', '--auto']), { split: 'right' });
    });
